...
function getSizeAtDistance(
  ...
  // Calculate the visible width using aspect ratio
  const aspect = size.width / size.height;
  const visibleWidth = visibleHeight * aspect;

  // Convert desired pixel size to world units
  const worldWidth = (widthInPixels / size.width) * visibleWidth;
  const worldHeight = (heightInPixels / size.height) * visibleHeight;
...
export default function InfoBox() {
  ...
  const htmlPosition = new THREE.Vector3(0, 0, 2); // Position of the HTML element relative to the group
  ...
  useFrame(() => {
    if (groupRef.current && htmlRef.current && contentRef.current) {
      ...
      const htmlGroup = groupRef.current.children[0] as THREE.Group;
      const truePosition = htmlGroup.getWorldPosition(new THREE.Vector3());

      // Get distance from camera to HTML element
      const htmlPos = new THREE.Vector3();
      groupRef.current.getWorldPosition(htmlPos);
      htmlPos.add(htmlPosition); // Add the local HTML position

      const cameraPos = new THREE.Vector3();
      camera.getWorldPosition(cameraPos);
      const distance = cameraPos.distanceTo(htmlPos);
      ...
      console.log("HTML element world dimensions:", {
        width: worldWidth,
        height: worldHeight,
        distance,
      });

      neighbourPosition.copy(truePosition);
      neighbourPosition.z += worldWidth + 0.5; // Adjust Z position to be behind the info box
      neighbourPosition.y += worldHeight / 2 + 0.5; // Adjust Y position to be above the info box
      console.log("Right side position:", neighbourPosition);

      if (!UIScene.three.infoBox.group.current) {
        UIScene.setInfoBox(
          groupRef,
          htmlRef,
          truePosition,
          new THREE.Euler(0, 0, 0),
          new THREE.Vector2(worldWidth, worldHeight)
        );
      } else {
        UIScene.setInfoBoxPosition(
          truePosition,
          new THREE.Euler(0, 0, 0),
          new THREE.Vector2(worldWidth, worldHeight)
        );
    ...
    <>
      <group position={position} ref={groupRef}>
        <Html
          ...
          //   occlusionTest={false}
          //   occlude={bilboardRef}
          className="pointer-events-none -translate-y-0.5"
        >
          <div
            className="absolute w-auto min-w-64 h-auto overflow-visible translate-x-1/2 "
            ref={contentRef}
          >
            <span className="text-black/50 absolute left-0 right-0 -mt-12 text-2xl  flex flex-row justify-between gap-2 items-center ">
              <p className="glass rounded-lg w-full p-0.5 pl-2">Title</p>
              <div className="flex h-10 gap-2 p-1 justify-center items-center  rounded-lg glass ">
                <Info />
                <CircleX />
              </div>
            </span>
            <div
              ...
              style={{
                ...
                transition: threeLineAnimationDone
                  ? "width 0.5s ease-in-out"
                  : "none",
              }}
            />

            {/* <div className="absolute  left-full bg-red-500 w-64 h-64"></div> */}
        ...
        />
        {/* the part preview */}
        <PartPreview position={neighbourPosition} />
      </group>
    </>
  );
}
